...
import { ADDRESS_FOLDER_KEY, DASHDOC_API_URL, USER_STORAGE_KEY } from 'src/app/services/constants';
...
export class AddressPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.addressService.fetchAddress().subscribe({
      next: (address) => {
        ...
        this.storage.get(ADDRESS_FOLDER_KEY).then(addressFolder => {
  ...
  async moveToFolder (modal: any, folder: string) {
    Object.values(this.selectedAddress).forEach ((address: any) => {
      ...
      this.storage.set (ADDRESS_FOLDER_KEY, this.addressFolder);
    });
