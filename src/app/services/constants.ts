export const DASHDOC_API_URL = 'https://api.dashdoc.eu/api/v4/';
export const API_URL = 'https://api.h24transports.com/api/';
export const API_URL_V2 = API_URL + 'carrier/';

export const CURRENT_COMPANY = 'CURRENT_COMPANY';

export const USER_STORAGE_KEY = 'USER_STORAGE';
export const JWT_KEY = 'JWT_KEY';
export const FIREBASE_TOKEN_KEY = 'FIREBASE_TOKEN';
export const TRANSPORTS_DRAFTS_KEY = 'TRANSPORTS_DRAFTS';
export const ADDRESS_FOLDER_KEY = 'ADDRESS_FOLDER';
export const ADDRESS_FOLDERS_KEY = 'ADDRESS_FOLDERS';
export const CONTACT_FOLDER_KEY = 'CONTACT_FOLDER';
export const COMPANIES_KEY = 'COMPANIES';

export const FILE_UPLOAD_MAX_SIZE = 1024 * 1024 * 10;
export const FILE_IMAGE_MAX_WIDTH = 1024;

export const HTTP_REQUEST_UNKNOWN_ERROR = 'Une erreur imprévue s\'est produite, veuillez contacter le support';
